...
int pWriteData(int dev, int endpoint, unsigned char *buf, unsigned long *length)
{
	int ret,pr;
	//length = (int *)length;
	ret =jtag_libusb_bulk_write((struct libusb_device_handle *)wfd, endpoint, (char *)buf, *((int *)length), 3000,&pr);
	if(ret==ERROR_OK)
		 return 1;
	else
		 return ret;
...
int pReadData(int dev, int endpoint, unsigned char *buf, unsigned long *length)
{
	...
	if (endpoint == 1)
	{
		ret=jtag_libusb_bulk_read((struct libusb_device_handle *)wfd, 0x81, (char *)buf, *((int *)length), 3000,&pr);
	}
	else
	{
		ret=jtag_libusb_bulk_read((struct libusb_device_handle *)wfd, 0x82, (char *)buf, *((int *)length), 3000,&pr);
	}
	if(ret==ERROR_OK)
		 return 1;
	else
